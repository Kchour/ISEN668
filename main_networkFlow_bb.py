...
dayHorizon = 5
schedule_limit = 5 # 5, 10, 50 limit the number of schedules generated, if using GEN method
if method == "GEN":
...
regions = dfRegions['Region'].tolist()[0:16]          #select some range     
...
# This is not used by the pyomo...returns numeric value
def obj_value(model):
    sum_ =  sum(valueMNRD[(m, n, r, d)]*model.level[m, n, r, d].value for m in missionTypes for n in missionNums for r in regions for d in days if (m, n, r, d) in model.level)
    return  sum_
...
# call solve which will put the solution in model automatically
results = opt.solve(model, tee=True)
# store the solution somewhere...how bout results?
model.solutions.store_to(results)
#model.solutions.load_from(results)  ...we can also load from somewhere
results.write()
...
##### Begin BB algorithm here
# test helper class for flooring
